...
class Binoculars(DetectorBase):
    ...
    def __init__(self,
                 observer_name_or_path='Qwen/Qwen2.5-3B',
                 performer_name_or_path='Qwen/Qwen2.5-3B-Instruct',
                 device: TorchDeviceMapType = 'cuda:0',
                 max_seq_length=512,
                 flash_attn=False,
                 quantization_bits=4,
                 **model_args):
    ...
    def _get_score_impl(self, text: t.Iterable[str]) -> torch.Tensor:
        ...
        # ROBUST HOTFIX: Move all tensors to CPU to prevent device mismatch errors
        observer_logits = observer_logits.cpu()
        performer_logits = performer_logits.cpu()
        input_ids = encodings.input_ids.cpu()
        attention_mask = encodings.attention_mask.cpu()

        log_ppl = seq_label_cross_entropy(performer_logits, input_ids, attention_mask)
